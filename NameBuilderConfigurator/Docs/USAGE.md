# NameBuilder Configurator – XrmToolBox UI Reference

This guide provides a comprehensive reference for using the **NameBuilder Configurator** in XrmToolBox. This tool provides a visual designer for creating and managing NameBuilder configurations without writing JSON manually.

**For getting started**: See the main [README.md](../../README.md) for a quick-start guide.

**For JSON configuration details**: See [NameBuilderPlugin/Docs](../../NameBuilderPlugin/Docs) for schema, examples, and patterns.

**For administrators**: See [ADMINISTRATOR.md](../../ADMINISTRATOR.md) for reviewing and managing plugin components.
...
1. **XrmToolBox** installed on your desktop
2. Active connection to your Dataverse environment
3. Administrator or System Customizer role in the environment

## 3. Installation

**Recommended**: Install via XrmToolBox Tool Library:

```text
Configuration ➜ Tool Library ➜ search "NameBuilder Configurator" ➜ Install
```

Restart XrmToolBox and the plugin appears in the tool list.

**For developers**: See [../../Docs/BUILDING.md](../../Docs/BUILDING.md) for building from source.
...
3. **Version Hash Verification**: The configurator compares the local plugin DLL hash with the server version. If a mismatch is detected, you'll see a warning indicating version incompatibility. Update either the local configurator tool or the server plugin to resolve this. Version mismatches can cause configuration errors or unexpected behavior.
4. **Solution Selection for Plugin Installation**: When installing or updating the NameBuilder plugin assembly, you'll be prompted to select an unmanaged solution. Only unmanaged solutions are shown to ensure compatibility with ALM workflows. Your selection is saved per connection.
5. Once validated, the utility caches the located `plugintype` so it can query/update the corresponding `sdkmessageprocessingstep` rows when you publish configuration changes.
6. **Component Organization**: All plugin components (assemblies and SDK message processing steps) are automatically added to your selected solution, enabling proper solution layering and transport between environments.
...
3. Click **Remove** to clear any previously configured alternate field or default value.
4. Save to push either of these outcomes: a nested `alternateField` element (when you selected another attribute, optionally with its own default), or a top-level `default` value (when you only supplied literal text).
...
**Solution Selection for Publishing**: When you click **Publish Configuration** for the first time (or after the plugin is installed), you'll be prompted to select an unmanaged solution where the NameBuilder steps will be registered. This selection is remembered per connection for future publishes.

**Benefits of Solution Selection**:

- **ALM-Ready**: All plugin components (assemblies, steps) are organized within your chosen solution
- **Environment Transport**: Easily move configurations between dev, test, and production via solution export/import
- **Solution Layering**: Supports proper layering in multi-solution environments
- **Version Control**: Track changes through solution version history
- **Deployment Isolation**: Keep NameBuilder components separate from other customizations

The tool ensures only one Create step and one Update step exist per entity, updating existing steps rather than creating duplicates.
...
## 7. Understanding the Generated JSON

The configurator generates JSON configurations that the NameBuilder plugin reads. For complete JSON schema documentation, see:

- **[JSON Schema Reference](../../NameBuilderPlugin/Docs/SCHEMA.md)** – Complete schema documentation
- **[Configuration Examples](../../NameBuilderPlugin/Docs/EXAMPLES.md)** – Sample configurations
- **[Conditional Fields](../../NameBuilderPlugin/Docs/CONDITIONAL_FIELDS.md)** – includeIf syntax and operators

### Quick JSON Overview

The JSON tab in the configurator shows the generated configuration with these key properties:

- `entity`: The logical name of the entity (e.g., `contact`, `account`)
- `targetField`: The destination field (typically `name`)
- `maxLength`: Global length limit (or auto-detected from metadata)
- `enableTracing`: Debug flag for Plugin Trace Log
- `fields`: Array of field blocks with:
  - `field`: Attribute logical name
  - `type`: Field type (`string`, `lookup`, `date`, `optionset`, `number`, `currency`)
  - `prefix`/`suffix`: Text before/after the value
  - `format`: Date/number formatting pattern
  - `maxLength`/`truncationIndicator`: Per-field truncation
  - `default`: Fallback text when empty
  - `alternateField`: Nested fallback field
  - `includeIf`: Conditional logic
...
- **[Main README](../../README.md)** – Getting started guide for users
- **[Administrator Guide](../../ADMINISTRATOR.md)** – Reviewing, troubleshooting, and managing components
- **[NameBuilder Plugin Docs](../../NameBuilderPlugin/Docs)** – JSON schema, examples, and patterns
- **[Developer Documentation](../../NameBuilderPlugin/README.md)** – Plugin architecture and building from source
- **[Build Scripts](../../Docs/BUILDING.md)** – Building the configurator from source
